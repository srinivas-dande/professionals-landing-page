  ...
  return (
    <section className="py-16 md:py-24 bg-white">
      <div className="max-w-[1200px] mx-auto px-4 md:px-8">
        ...
        {/* Tabs */}
        <div className="bg-[#F9FAFB] rounded-xl p-2 mb-12">
            ...
            {tabs.map((tab, index) => (
              <button
                ...
                className={`flex-1 py-4 px-6 text-center font-semibold transition-all relative ${
                  activeTab === index
                    ? 'text-[#111827] bg-white rounded-lg shadow-sm'
                    : 'text-[#6B7280] hover:text-[#111827]'
                }`}
              >
                {tab.label}
                {activeTab === index && (
                  <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-32 h-1 bg-[#CF2030] rounded-full" />
                )}
              </button>
            ))}
          </div>
